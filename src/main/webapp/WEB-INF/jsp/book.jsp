                ...
                <tr>
                    <th height="50px">
                        <form method="post" action="/book?id=${book.id}">
                            <input type="submit" value="Заказать" class="btn float-left login_btn"
                                   style="width: 200px;height:45px;background:orange;">
                            <br><br>
                ...
                <tr>
                    ...
                    <th height="50px">
                        ...
                        <c:if test="${periodic eq null}">
                            <c:if test="${book.authors.size() ne 0}">
                                <p>Авторы:
                        ...
                        <c:if test="${periodic ne null}">
                            ...
                            <ul>
                                <c:forEach items="${periodic.articles}" var="article">
                                    <li>
                                            ${article.name}
                                        <c:if test="${article.authors.size() ne 0}">
                                            -
                                            <c:forEach items="${article.authors}" var="author">
                                                <c:if test="${author.id eq article.authors.get(article.authors.size()-1).id}">
                                                    ${author.name}.
                                                </c:if>
                                                <c:if test="${author.id ne article.authors.get(article.authors.size()-1).id}">
                                                    ${author.name},
                                                </c:if>
                                            </c:forEach>
                                        </c:if>
